...
def make_map(x,y,z,crs_in,save_name,map_projection="original",colormap="jet",
    ...
    """
    create and save a pdf map of a 2D variable.
    ...
    save_name: the path w/ basename of the output file with extension
    ...
    cont = ax.pcolormesh(x,y,z,cmap=colormap,rasterized=True)
    ...
    if draw_scalebar:
        ...
        if ax_projection == "rectilinear":
            print(" Warning: scalebar option may not work with rectilinear projection.")
            sb_proj = None
        else:
...
def add_feature(ax,shp_file,crs="EPSG:4326",zorder=0.5,color="gray",width=1.5):
    ...
    map is assumed lat lon if crs is not defined.
    """
...
def read_netcdf_2D(nc_fname,nc_xvar,nc_yvar,nc_var,nc_tdim=None,nc_ntime=0,
    ...
    nc_fname: is the netcdf file name
    nc_xvar, nc_yvar: are the names of the netcdf variables of x/y coordinates
    nc_var: is the name of the variable we want to extract (4D)
    nc_tdim: is the name of the netcdf dimension time
    nc_ntime: is the time step from which to extract the values of nc_var to plot
    nc_btdim: name of nc bottom_top dimension
    nc_nlevel: is the height (int) at which to extract var
    nc_crsdim: name of nc attribute with proj string of netcdf coordinate ref system
    """
    ...
    if nc_crsdim in data.attrs:
        ...
        crs = None
        print("Could not find the CRS of the netcdf file.")
    ...
    if (z.ndim == 4 and (nc_tdim == None or nc_btdim == None)):
        ...
        print(__file__ + "nc_btdim or nc_tdim are defined,but array is 2D.")
    # subsample z
    if nc_tdim != None:
